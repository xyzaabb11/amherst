...
class MDict(object):
    ...
    def _split_key_block(self, key_block):
        ...
        while key_start_index < len(key_block):
            ...
            key_text = key_block[key_start_index+self._number_width:key_end_index].decode(self._encoding, errors='ignore').strip()
    ...
    def _read_header(self):
        ...
        header_text = f.read(header_text_size)[:-2].decode('utf-16')
    ...
    def _read_keys(self):
        ...
        if key_block_info_list:
            ...
            for key_block in key_block_compressed.split(b'\x02\x00\x00\x00')[1:]:
...
if __name__ == '__main__':
    ...
    if (os.path.exists(mdd_filename)):
        ...
        #else:
        #    fname = mdd_filename
        log.info('filename: %s'%mdd_filename)
        log.info('number of Entries: %s'%len(mdx))
        for key, value in mdx.header.items():
            log.debug('%s: %s'%(key, value))
    else:
